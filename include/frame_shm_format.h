 ...
 * low_water_permille (ring occupancy, 0 = drained). Same layout, OPPOSITE
 * polarity -- exactly what an offset-addressing consumer cannot detect for
 * itself, hence a version bump. We never defined offset 88, so nothing below
 * changes; the constant must still move or shm_ingress.c refuses every v2
 * ring. */
