...
5. **Frame Release Tab:** A **cadence (Gantt-style) timeline** of how the link delivers frames against the expected frame period. X is wall-clock over a selectable **1 / 2 / 5 / 10 s** window (newest at the right); faint vertical gridlines mark the measured frame period (the metronome). Each frame is a horizontal bar spanning its first packet → marker (its *span*), colored **green** (on time), **amber** (late — completion drifted past half a frame period), or **red** (its packets shared a release burst with the previous frame = a cross-frame burst, the drop trigger). The healthy pattern reads as short green bars hugging the left edge of each gridline cell with silence between; problems show as bars drifting right, spanning a gridline, or turning red. A frames-touched histogram (1 / 2 / 3 / 4+) and a summary line (chunk count, overlap count and rate, average packets- and frames-per-chunk) quantify how often bursts straddle frame boundaries. The **Gap (µs)** control sets the idle-gap threshold that separates one release burst from the next — tune it to your link's FEC block cadence. Enable, Pause, and Reset mirror the Frame Blocks controls.
