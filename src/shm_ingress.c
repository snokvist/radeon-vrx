...
static void push_frame(ShmIngress *si, const uint8_t *data, size_t len,
    ...
    GstAppSrc *appsrc = !skip_delta &&
                       (si->push_enabled || start_stream) && si->appsrc
                      ? GST_APP_SRC(gst_object_ref(si->appsrc)) : NULL;
    if (start_stream) {
        si->waiting_for_idr = FALSE;
        si->stream_reset_pending = FALSE;
        /* Ensure the random-access unit is accepted even if enough-data from
         * the previous stream left flow control closed. */
